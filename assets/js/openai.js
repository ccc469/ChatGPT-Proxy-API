...
const app = {
  ...
  clear() {
    ...
    this.total = this.total.filter((item) => {
      return (
        ...
        (item.total_available > 0 || item.total_available === "查询失败")
      );
    });
  ...
  async submit($refs) {
    if (!this.u) {
      this.alert = {
        type: "error",
        message: "请输入以sess-开头的Authorization...",
      };
      return;
    }
  ...
  async checkBilling(key) {
    ...
    const subscription = await this.fetch(
    ...
    if (!subscription || !subscription.plan) {
      ...
      const start_date =
        subscription.hard_limit_usd > 20
          ? [today.getFullYear(), today.getMonth() + 1, "1"].join("-")
          : formatDate(today / 1000 - 90 * 86400);
      const end_date = formatDate(today / 1000 + 86400);
      const usageData = await this.fetch(
      ...
      const gptModels = modelData.data.filter((model) =>
      ...
      const plan =
        subscription.plan.title === "Pay-as-you-go"
          ? "Pay-as-you-go"
      ...
      const total_used =
        typeof usageData.total_usage === "number"
          ? usageData.total_usage / 100
          : "查询失败";

      // 剩余额度
      const total_available =
        typeof total_used === "number"
